...
import { Search, ChevronRight, ChevronLeft, Book } from 'lucide-react';
import BookReader from './BookReader';
import { safeFetch } from '../utils/fetchUtils';

interface HisbahBook {
  shamela_id: string;
  title_ar: string;
  author: string;
  full_content: string;
...
export default function HisbahLibrary({ language, theme, onBack }: { language: 'ar' | 'ku' | 'en', theme: 'night' | 'white' | 'cream', onBack: () => void }) {
  const [books, setBooks] = useState<HisbahBook[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedBook, setSelectedBook] = useState<HisbahBook | null>(null);
  const [currentPage, setCurrentPage] = useState(1);
  const booksPerPage = 24;

  useEffect(() => {
    safeFetch<HisbahBook[]>('https://raw.githubusercontent.com/sherwanymuhammad82-svg/my-book-api/refs/heads/main/ALL_Hishba_Books_Content.json')
      .then(data => {
        setBooks(data);
        setIsLoading(false);
      })
      .catch(err => {
        console.error("Error fetching Hisbah books:", err);
        setIsLoading(false);
      });
  }, []);
    ...
    <div className="p-6 max-w-md mx-auto w-full pb-24">
      ...
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-olive to-gold mb-2 font-serif">
          {language === 'ar' ? 'مكتبة الحسبة' : language === 'en' ? 'Hisbah Library' : 'کتێبخانەی حیسبە'}
        </h2>
      </div>
